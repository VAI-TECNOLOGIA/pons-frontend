...
export default function Corretores() {
 ...
 const isAtivo = (c.status === 'ATIVO' || c.ativo) && c.userActive !== false;
 return (
 <tr key={c.id}>
 <td>
...
function TrocarSenhaModal({ corretor, onClose }: { corretor: any; onClose: () => void }) {
 ...
 <input className="field__input" type={mostrar ? 'text' : 'password'} value={senha} onChange={(e) => setSenha(e.target.value)} minLength={6} required autoFocus placeholder="mínimo 6 caracteres" autoComplete="new-password" style={{ paddingRight: 44, width: '100%' }} />
 <button type="button" onClick={() => setMostrar((v) => !v)} aria-label={mostrar ? 'Ocultar senha' : 'Mostrar senha'} title={mostrar ? 'Ocultar senha' : 'Mostrar senha'} style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', background: 'transparent', border: 'none', cursor: 'pointer', padding: 4, color: 'currentColor', opacity: 0.7, display: 'inline-flex' }}>
 {mostrar ? (
