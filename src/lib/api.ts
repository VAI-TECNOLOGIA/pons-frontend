...
export const Api = {
  ...
  equipesMinhas: () => request<{ admin: boolean; equipeIds: number[] }>('/equipes/minhas'),
  equipeAtribuirLider: (equipeId: number, userId: number) =>
    request<{ ok: boolean }>(`/equipes/${equipeId}/lider`, { method: 'POST', body: { userId } }),
  ...
  empreendimentoFotoComentario: (id: number, fotoId: number, comentario: string) =>
    request<{ ok: boolean; iaEnvia: boolean; iaComentario: string | null }>(`/empreendimentos/${id}/fotos/${fotoId}/ia`, { method: 'POST', body: { comentario } }),
  ...
  vendasAnalytics: (params?: Record<string, any>) => request<any>('/vendas/analytics' + qs(params)),
  ...
  participacaoGet: (unidadeId: number) => request<{ itens: any[]; soma: number; ok: boolean }>(`/unidades/${unidadeId}/participacao`),
  participacaoSet: (unidadeId: number, itens: any[]) => request<{ itens: any[] }>(`/unidades/${unidadeId}/participacao`, { method: 'PUT', body: { itens } }),
  ...
  whatsappTemplateCreate: (body: {
    ...
    comLogo?: boolean; headerDocument?: boolean; botao?: { text: string; url: string };
  }) =>
